...
SELECT name, monthlymaintenance, CASE WHEN (monthlymaintenance > 100)
		THEN 'expensive' ELSE 'cheap' END AS cost_classification
			FROM Facilities
...
SELECT DISTINCT CONCAT(m.surname, ', ', m.firstname) AS full_name,
	f.name AS court_name FROM Members m 
...
SELECT CASE WHEN m.surname = 'GUEST' THEN m.surname
		ELSE CONCAT(m.surname, ', ', m.firstname) END AS full_name, 
	...
	AND AND ((m.memid = 0 AND b.slots * f.guestcost > 30)
